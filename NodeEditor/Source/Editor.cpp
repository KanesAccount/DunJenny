...
void ed::EditorContext::Begin(const char* id, const ImVec2& size)
{
   ...
   ImGui::LogToClipboard();
    ...
    // #debug -> BIG PINK BOX ALL OVER PLCE
	//auto drawList2 = ImGui::GetWindowDrawList();
	//drawList2->PushClipRectFullScreen();
	//drawList2->AddRect(m_Canvas.FromClient(ImVec2(0, 0)), m_Canvas.FromClient(ImVec2(0, 0)) + //m_Canvas.ClientSize,  IM_COL32(255, 0, 255, 255), 0, 15, 4.0f);
    ...
    // #debug #clip -> TEXT IN TOP LEFT OF GRID
...
void ed::EditorContext::End()
{
...

    {
        ...
        // #debug: Static grid in local space -> BBIG RED GRID in grey grd
		//for (float x = 0; x < m_Canvas.WindowScreenSize.x; x += 100)
        //   drawList->AddLine(ImVec2(x, 0.0f) + m_Canvas.WindowScreenPos, ImVec2(x, //m_Canvas.WindowScreenSize.y) + m_Canvas.WindowScreenPos, IM_COL32(255, 0, 0, 128));
		//for (float y = 0; y < m_Canvas.WindowScreenSize.y; y += 100)
        //   drawList->AddLine(ImVec2(0.0f, y) + m_Canvas.WindowScreenPos, ImVec2(m_Canvas.WindowScreenSize.x, y) + m_Canvas.WindowScreenPos, IM_COL32(255, 0, 0, 128));
    }
...
void ed::EditorContext::ClearScreen(bool t)
{
	m_ClearCommand = t;
}
...
void ed::DeleteItemsAction::Clear()
{
	m_CandidateObjects = m_ManuallyDeletedObjects;
}

void ed::DeleteItemsAction::EmptyClear()
{
	m_CandidateObjects.clear();
	m_ManuallyDeletedObjects.clear();
}
...
{
	if (!m_IsActive && !m_ClearScreen)
	{
		return false;
	}
	else if (m_ClearScreen)
	{
