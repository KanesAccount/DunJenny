...
static int BuildIdList(C& container, I* list, int listSize, F&& accept)
{
    if (list != nullptr)
    {
        ...
        for (auto object : container)
        {
            ...
            if (accept(object))
            {
                list[count] = I(object->ID().ToPointer());
                ++count;
                --listSize;
            }
        }
...
void ax::NodeEditor::ClearScreen(bool t)
{
	if (t)
	{
		s_Editor->GetItemDeleter().Clear();
		s_Editor->GetItemDeleter().setClear(t);
	}
	else
	{
		s_Editor->GetItemDeleter().EmptyClear();
		s_Editor->GetItemDeleter().setClear(t);
	}
}
